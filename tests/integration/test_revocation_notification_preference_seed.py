"""Integration test for migration 029: seeds token_revocation.failed
preferences for admins who already have a Telegram link (issue #142 N4).
...
fixture (already migrated to head, including revision 029) shared by the
...
_MIGRATION_PATH = (
    ...
    / "alembic"
    / "versions"
    / "029_seed_token_revocation_failed_preferences.py"
)


def _load_migration() -> ModuleType:
    spec = importlib.util.spec_from_file_location("revision_029", _MIGRATION_PATH)
    ...
    async with db_engine.connect() as conn:
        ...
        try:
            ...
            count_query = text(
                ...
                "WHERE user_id = :user_id AND notification_class = 'token_revocation.failed'"
            )
