...
async def test_command_sweep_timeout_bumps_revision_and_emits_operation_update(
    ...
    ``publish_operation_event`` runs after the command-expiry transaction commits.
    """
...
async def test_command_sweep_orphan_bumps_revision_and_emits_operation_update(
    db_engine,
) -> None:  # type: ignore[no-untyped-def]
    """A queued command orphaned by a terminal session reaches its owner once.

    ``publish_operation_event`` runs after the orphan-cancellation transaction commits.
    """
    session_factory = async_sessionmaker(bind=db_engine, expire_on_commit=False)
    user = User(
        id=uuid4(),
        email=f"command-orphan-{uuid4().hex}@example.com",
        password_hash="hash",
        product_id="vex",
    )
    gpu_session = GpuSession(
    ...
    try:
        ...
        assert publish_call.kwargs["user_id"] == user.id
        assert publish_call.kwargs["event_type"] == EventType.GPU_SESSION_OPERATION_UPDATED
        assert publish_call.kwargs["payload"].id == operation_id
        assert publish_call.kwargs["payload"].revision == 1
    finally:
        async with session_factory() as db:
            await db.execute(delete(User).where(User.id == user.id))
...
async def test_command_sweep_does_not_emit_when_terminal_operation_rejects_close(
    db_engine,
) -> None:  # type: ignore[no-untyped-def]
    """A close_failed guard miss cannot create a discarded SSE frame."""
    session_factory = async_sessionmaker(bind=db_engine, expire_on_commit=False)
    user = User(
        id=uuid4(),
        email=f"command-orphan-terminal-{uuid4().hex}@example.com",
        password_hash="hash",
        product_id="vex",
    )
    gpu_session = GpuSession(
    ...
    try:
        async with session_factory() as db, db.begin():
            ...
            assert await GpuSessionOperationRepository(db).close_failed(
                operation.id, at=datetime.now(UTC), error="already terminal"
            )
            await GpuSessionCommandRepository(db).create(
                id=command_id,
                session_id=gpu_session.id,
