...
async def test_get_active_user_returns_active(
    user_repo: UserRepository, make_user
) -> None:
...
async def test_get_active_user_by_email_found(
    user_repo: UserRepository, make_user
) -> None:
...
async def test_email_exists_true_for_existing(
    user_repo: UserRepository, make_user
) -> None:
...
async def test_email_exists_false_for_inactive(
    user_repo: UserRepository, make_user
) -> None:
...
async def test_email_exists_excludes_given_user(
    user_repo: UserRepository, make_user
) -> None:
...
async def test_soft_delete_user_sets_inactive(
    user_repo: UserRepository, make_user
) -> None:
...
async def test_mark_email_verified_sets_timestamp(
    user_repo: UserRepository, make_user
) -> None:
...
async def test_get_refresh_token_by_hash_found(
    user_repo: UserRepository, make_user
) -> None:
...
async def test_get_valid_refresh_token_returns_active(
    user_repo: UserRepository, make_user
) -> None:
...
async def test_get_valid_refresh_token_returns_none_for_revoked(
    user_repo: UserRepository, make_user, db_session: AsyncSession  # noqa: ARG001
) -> None:
...
async def test_revoke_refresh_token_returns_true(
    user_repo: UserRepository, make_user
) -> None:
...
async def test_cleanup_expired_tokens_deletes_expired(
    user_repo: UserRepository, make_user
) -> None:
...
async def test_set_and_get_preferred_billing_account(
    user_repo: UserRepository, make_user
) -> None:
...
async def test_get_user_job_count_empty(
    user_repo: UserRepository, make_user
) -> None:
...
async def test_get_user_output_count_empty(
    user_repo: UserRepository, make_user
) -> None:
...
async def test_get_user_upload_count_empty(
    user_repo: UserRepository, make_user
) -> None:
...
async def test_get_user_storage_bytes_empty(
    user_repo: UserRepository, make_user
) -> None:
...
# ---------------------------------------------------------------------------
# get_first_outputs_for_jobs
# ---------------------------------------------------------------------------


async def _make_output_row(
    ...
    *,
    ...
    """Helper: persist a minimal GenerationOutput and return it."""
    from uuid import uuid4

    out_id = uuid4()
    return await storage_repo.create_output(
        id=out_id,
        user_id=user_id,
        job_id=job_id,
        storage_key=storage_key or f"users/{user_id}/outputs/{job_id}/{out_id}.jpg",
...
async def test_get_first_outputs_for_jobs_empty_input(
    user_repo: UserRepository,
) -> None:
    """Returns an empty dict when job_ids list is empty."""
    result = await user_repo.get_first_outputs_for_jobs([])
    assert result == {}


async def test_get_first_outputs_for_jobs_no_outputs(
    user_repo: UserRepository,
    make_job,
) -> None:
    """Returns an empty dict when jobs have no outputs."""
    job = await make_job()
    result = await user_repo.get_first_outputs_for_jobs([job.id])
    assert result == {}


async def test_get_first_outputs_for_jobs_single_output(
    user_repo: UserRepository,
    storage_repo: StorageRepository,
    make_job,
) -> None:
    """Returns the one output that exists for the job."""
    job = await make_job()
    output = await _make_output_row(storage_repo, user_id=job.user_id, job_id=job.id)

    result = await user_repo.get_first_outputs_for_jobs([job.id])

    assert job.id in result
    assert result[job.id].id == output.id


async def test_get_first_outputs_for_jobs_returns_lowest_index(
    user_repo: UserRepository,
    storage_repo: StorageRepository,
    make_job,
) -> None:
    """When a job has multiple outputs, the one with output_index=0 is returned."""
    job = await make_job()
    out0 = await _make_output_row(
        storage_repo, user_id=job.user_id, job_id=job.id, output_index=0
    )
    await _make_output_row(
        storage_repo, user_id=job.user_id, job_id=job.id, output_index=1
    )
    await _make_output_row(
        storage_repo, user_id=job.user_id, job_id=job.id, output_index=2
    )

    result = await user_repo.get_first_outputs_for_jobs([job.id])

    assert result[job.id].id == out0.id


async def test_get_first_outputs_for_jobs_multiple_jobs(
    user_repo: UserRepository,
    storage_repo: StorageRepository,
    make_job,
    make_user,
) -> None:
    """Returns one output per job when given multiple job IDs."""
    user = await make_user(email=f"multi-{uuid4().hex[:6]}@example.com")
    job_a = await make_job(user=user)
    job_b = await make_job(user=user)
    job_c = await make_job(user=user)

    out_a = await _make_output_row(storage_repo, user_id=user.id, job_id=job_a.id)
    out_b = await _make_output_row(storage_repo, user_id=user.id, job_id=job_b.id)
    # job_c has no outputs

    result = await user_repo.get_first_outputs_for_jobs([job_a.id, job_b.id, job_c.id])

    assert len(result) == 2
    assert result[job_a.id].id == out_a.id
    assert result[job_b.id].id == out_b.id
    assert job_c.id not in result


async def test_get_first_outputs_for_jobs_unknown_ids_ignored(
    user_repo: UserRepository,
) -> None:
    """Job IDs that don't exist in the DB don't appear in the result."""
    result = await user_repo.get_first_outputs_for_jobs([uuid4(), uuid4()])
    assert result == {}
