...
- Response: `LegalDocumentResponse`, with headers `ETag: "<sha256>"` and `Cache-Control: public, max-age=300`.
...
- Idempotent: already-accepted current versions are skipped server-side.
...
2. On `428 legal_acceptance_required`:
   ...
   2. For each one, `GET /v1/legal/documents/{doc_type}` returns the text to render and the version and sha256 to echo back.
