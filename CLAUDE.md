...
14. **Legal documents & acceptance** — versioned markdown in `legal/{product}/{doc_type}/{YYYY-MM-DD}.md`, declared in `legal/manifest.toml` (strict bijection, strictly increasing versions, first version must set `requires_reacceptance`, and every entry pins the normalized-content SHA-256). **Published versions are immutable:** change text by publishing a new dated file, never editing an accepted version in place. Version = effective date (UTC); "current" = latest `version <= today`; "required" = latest effective version with `requires_reacceptance`. `LegalDocumentRegistry` is built once in `init_services()` (a hash mismatch, an unsatisfiable `ProductConfig.required_legal_documents`, or — in production — a `DRAFTING NOTE`/unresolved bracket placeholder fails startup) and exposed on `app.state["legal_registry"]`. Acceptances are append-only rows in `legal_acceptances` (version + `content_sha256` + IP + UA + source); sensitive-data consent withdrawal is recorded only by `DELETE /v1/users/me`.
...
- **Media ingest and PDQ ledger** (`src/api/services/media_ingest/`, `src/api/services/media_hash_ledger.py`, `media_hashes`): every new original is prepared before R2 by the process-local shared ingestor. It strips unapproved container metadata, bakes static orientation, and hashes the exact stored image/video samples under immutable profiles. ICC/colour/aspect and compressed rendering data are functional exceptions, so “sanitized” does not mean every identifying byte is removed. Original row → ledger rows → mandatory flush must occur before each optional thumbnail savepoint. Ledger rows survive content retention/deletion, cascade with a hard user deletion, and null their job relation on hard job deletion. See `docs/designs/media-ingest-pdq-ledger.md` for format, animation, failure, and transaction details.
